...
The function body must be a **single bare expression** — no trailing semicolon,
no intermediate `let` bindings. (Let binding support is not yet implemented.)
...
- Only `powi` for powers. `powf` and anything not in the table above will
  cause a compile-time panic.
- No `let` bindings in the function body yet.
- No current support for higher-order derivatives.
